...
The next step wants us to drop all words that contain the character sequence `ugh` followed with any number of h's then punctuation. We will do so by first using `str_detect` to check whether each element in `message` meets the deletion criterion by using the regex repetition operator `+` and the regex punctuation character class `[:punct:]` - this operation returns an array of booleans that tells us if each element meets the condition. We then flip every boolean in the array, meaning now the indices corresponding to the elements that **don't** meet the deletion criterion will say `TRUE`. Finally, we use this boolean array to subset `message`.
```{r remove ugh}
message <- message[!str_detect(message, "ugh+[:punct:]")]
message
```
