...
# str_subset(crsl$artist, "Jimi Hendrix")[c(TRUE,
#   rep(FALSE, length(str_subset(crsl$artist, "Jimi Hendrix")) - 1))]

crsl[["artist"]][crsl$artist == "Jimi Hendrix"][1]
class(crsl[["artist"]][crsl$artist == "Jimi Hendrix"][1])
```

Here is the second implementation - we extract the `artist` column as a character vector using `[[]]`, then use a song name ("Red House") to match the correct row.

```{r extract "Jimi Hendrix" by taking values from the dataframe}
crsl[["artist"]][crsl == "Red House"]
crsl[["artist", crsl$artist=="Jimi Hendrix"]][1]
```
